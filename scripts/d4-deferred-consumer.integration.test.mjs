...
test('packed production smoke has an absolute preview baseURL and authentic collector output', async () => {
  ...
  assert.ok(report.familyCoverage.TreeSelect.scenarios.some(scenario => scenario.logicalSearchMatches >= 5000 && scenario.controlledRejected === true && scenario.eventRecords.some(event => event.name === 'check')))
