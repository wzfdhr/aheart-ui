...
test('packed production smoke has an absolute preview baseURL and authentic collector output', async () => {
  ...
  assert.ok(report.familyCoverage.Tree.realData && report.familyCoverage.TreeSelect.realData && report.familyCoverage.Cascader.realData)
  assert.ok(report.familyCoverage.Tree.actions.includes('expand'))
  assert.ok(report.familyCoverage.TreeSelect.actions.includes('check') && report.familyCoverage.TreeSelect.actions.includes('search'))
  assert.ok(report.familyCoverage.Cascader.actions.includes('keyboard') && report.familyCoverage.Cascader.actions.includes('lazy'))
  ...
  assert.equal(report.ssrHydration.initialWindowDeterministic, true)
  assert.equal(report.ssrHydration.idsDeterministic, true)
  assert.equal(report.ssrHydration.postHydrationInteraction, true)
  ...
  assert.equal(report.realEvidenceBinding.buildFingerprintVerified, true)
  assert.equal(report.realEvidenceBinding.moduleFingerprintVerified, true)
