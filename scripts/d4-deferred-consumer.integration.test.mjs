...
test('real bounded iframe lifecycle evidence has authentic realm, popup, unmount and late-loader records', async t => {
  ...
  await t.test('scenario matrix, surface descriptors and raw artifact', async () => {
    ...
    for (const scenario of raw.scenarios) { const parsed = new URL(`http://iframe.test${scenario.scenarioUrl}`); assert.equal(parsed.searchParams.get('iframeProbe'), 'true'); assert.equal(parsed.searchParams.get('virtual'), 'true'); assert.equal(parsed.searchParams.get('rowMode'), 'fixed'); if (scenario.component === 'Cascader') assert.equal(parsed.searchParams.get('cascaderScenario'), 'iframe-lazy') }
  ...
  await t.test('resource constructor events are raw and drained', () => {
    ...
    for (const scenario of raw.scenarios) { const install = scenario.events.find(event => event.type === 'instrumentation-install'); assert.ok(install && install.installedBeforeMount && install.realmType === 'iframe' && install.collectorWaitsExcluded); assert.deepEqual(install.proxyKinds.sort(), ['interval', 'raf', 'resizeObserver', 'timeout']); assert.ok(scenario.events.every((event, index) => event.seq === index + 1 && event.time >= (scenario.events[index - 1]?.time ?? 0) && event.scenarioId === scenario.scenarioId && event.realmId === scenario.realmId)); const unmount = scenario.events.findIndex(event => event.type === 'frame-unmount-invoked'); for (const kind of ['resizeObserver', 'raf', 'timeout', 'interval']) { const records = scenario.events.filter(event => event.type === 'resource' && event.kind === kind); assert.ok(records.length > 0); assert.ok(records.every(event => event.source === 'component-runtime' && event.resourceId && event.targetSelector && event.realmId === scenario.realmId)); assert.ok(records.every((event, index) => event.action === 'create' || event.action === 'observe' || event.action === 'callback' || scenario.events.indexOf(event) > unmount)); } }
  ...
  await t.test('popup focus Teleport unmount and late loader events are real', () => {
    const raw = assertRaw(); for (const component of ['TreeSelect', 'Cascader']) { const events = raw.scenarios.find(scenario => scenario.component === component).events; for (const type of ['popup-open', 'escape', 'popup-close', 'focus-restore', 'reopen']) assert.ok(events.some(event => event.type === type && event.realmId === 'iframe')); assert.ok(events.some(event => event.type === 'popup-open' && event.ownerDocument === true && event.defaultView === true)) }
    const cascader = raw.scenarios.find(scenario => scenario.component === 'Cascader').events; for (const type of ['lazy-pending', 'lazy-abort', 'lazy-resolve-after-unmount']) assert.ok(cascader.some(event => event.type === type)); for (const component of ['TreeSelect', 'Cascader']) assert.ok(raw.scenarios.find(scenario => scenario.component === component).events.filter(event => ['popup-open', 'escape', 'popup-close', 'focus-restore', 'reopen'].includes(event.type)).every(event => event.realmId === raw.scenarios.find(scenario => scenario.component === component).realmId)); assert.equal(raw.summary?.teleportResidualNodes, 0); assert.equal(raw.summary?.escapeFocusRestored, true); assert.equal(raw.summary?.unmountCleanup, true); assert.equal(raw.summary?.lateLazyStateUpdates, 0); assert.equal(raw.summary?.postUnmountInteractions, 0)
  })
})
