...
test('real bounded iframe lifecycle evidence has authentic realm, popup, unmount and late-loader records', async t => {
  ...
  await t.test('popup focus Teleport unmount and late loader events are real', () => {
    ...
    const cascader = assertScenario(raw, 'Cascader').events; for (const type of ['lazy-pending', 'lazy-abort', 'lazy-resolve-after-unmount']) assert.ok(cascader.some(event => event.type === type)); const late = cascader.find(event => event.type === 'lazy-resolve-after-unmount'); assert.ok(late.stateRawBefore && late.stateRawAfter && late.domRawBefore && late.domRawAfter && late.callbacksBefore && late.callbacksAfter); assert.equal(late.componentUpdateCount, 0); assert.equal(late.stateHashBeforeSha256, hash(Buffer.from(late.stateRawBefore))); assert.equal(late.stateHashAfterSha256, hash(Buffer.from(late.stateRawAfter))); assert.equal(raw.summary?.teleportResidualNodes, 0); assert.equal(raw.summary?.escapeFocusRestored, true); assert.equal(raw.summary?.unmountCleanup, true); assert.equal(raw.summary?.lateLazyStateUpdates, 0); assert.equal(raw.summary?.postUnmountInteractions, 0)
  })
})
