...
test('real bounded iframe lifecycle evidence has authentic realm, popup, unmount and late-loader records', async t => {
  ...
  await t.test('resource constructor events are raw and drained', () => {
    ...
    const resources = []
    for (const component of components) { const scenario = assertScenario(raw, component); const install = scenario.events.find(event => event.type === 'instrumentation-install'); assert.ok(install && install.installedBeforeMount && install.realmType === 'iframe' && install.collectorWaitsExcluded); assert.deepEqual([...install.proxyKinds].sort(), ['interval', 'raf', 'resizeObserver', 'timeout']); assert.deepEqual([...install.installedProxyKinds].sort(), ['interval', 'raf', 'resizeObserver', 'timeout']); assert.equal(install.propertyLocked, true); assert.ok(install.probeMarker); assert.ok(scenario.events.every((event, index) => event.seq === index + 1 && event.time >= (scenario.events[index - 1]?.time ?? 0) && event.scenarioId === scenario.scenarioId && event.realmId === scenario.realmId)); resources.push(...scenario.events.filter(event => event.type === 'resource')) }
    assert.ok(resources.length > 0); assert.ok(resources.every(event => ['resizeObserver', 'raf', 'timeout', 'interval'].includes(event.kind) && ['create', 'observe', 'unobserve', 'callback', 'cancel', 'clear', 'disconnect'].includes(event.action) && event.resourceId && event.source === 'component-runtime' && event.realmId)); for (const scenario of raw.scenarios) { const observed = new Set(); for (const event of scenario.events.filter(event => event.type === 'resource')) { if (event.action === 'observe') observed.add(`${event.resourceId}:${event.targetSelector}`); if (event.action === 'unobserve') { assert.ok(observed.has(`${event.resourceId}:${event.targetSelector}`)); assert.equal(observed.size > 0, true) } if (event.action === 'disconnect') assert.ok(event.kind === 'resizeObserver') } }
