...
const currentLean = 'http://127.0.0.1:5173/assets/components_select.md.abc123.lean.js'
const differentHashLean = 'http://127.0.0.1:5173/assets/components_select.md.other456.lean.js'
...
test('VitePress cancellation classifier requires trace-backed prefetch evidence', () => {
  ...
  assert.equal(isIgnorableCancelledVitePressPrefetch('desktop-webkit', metadata(), request(current), 'Load request cancelled', new Set(['components_select:abc123'])), true)
  assert.equal(isIgnorableCancelledVitePressPrefetch('desktop-webkit', metadata(), request(currentLean), 'Load request cancelled', new Set()), false)
  assert.equal(isIgnorableCancelledVitePressPrefetch('desktop-webkit', metadata(), request(differentHashLean), 'Load request cancelled', new Set(['components_select:abc123'])), false)
