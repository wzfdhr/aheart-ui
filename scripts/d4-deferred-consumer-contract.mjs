...
export function recomputeActionability(timing) {
  ...
  return Boolean(timing && timing.startedAt < timing.triggerAt && timing.clickStartedAt <= timing.clickCompletedAt && timing.clickCompletedAt < timing.actionableAt && timing.triggerAt < timing.actionableAt && timing.actionableAt <= timing.nextTickAt && timing.nextTickAt <= timing.rafAt?.[0] && timing.rafAt?.[0] <= timing.rafAt?.[1] && timing.endAt === timing.rafAt?.[1] && timing.firstInteractionMs === timing.endAt - timing.startedAt && timing.probeAt >= timing.actionableAt && timing.probeAt <= timing.endAt && timing.targetSelectorIncludesTrigger === false && timing.targetKind === 'row' && timing.fallbackTarget === false && timing.hitTarget?.kind === 'row' && timing.focusProbe?.activeElementInRow === true && intersects && rect.enabled === true && rect.pointerEvents !== 'none' && timing.actionProbe?.hitTest === true && timing.actionProbe?.focused === true && timing.actionProbe?.ownerDocument === true)
}
