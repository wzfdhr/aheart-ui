...
export function recomputeActionability(timing) {
  ...
  return Boolean(timing && timing.startedAt <= timing.triggerAt && timing.clickStartedAt <= timing.clickCompletedAt && timing.clickStartedAt - timing.startedAt <= 5 && timing.clickCompletedAt < timing.actionableAt && timing.triggerAt <= timing.actionableAt && timing.actionableAt <= timing.nextTickAt && timing.nextTickAt <= timing.rafAt?.[0] && timing.rafAt?.[0] <= timing.rafAt?.[1] && timing.rafAt?.[1] <= timing.probeAt && timing.probeAt <= timing.endAt && timing.firstInteractionMs === timing.endAt - timing.startedAt && timing.targetSelectorIncludesTrigger === false && timing.targetKind === 'row' && timing.fallbackTarget === false && timing.hitTarget?.kind === 'row' && timing.focusProbe?.activeElementInRow === true && intersects && rect.enabled === true && rect.pointerEvents !== 'none' && timing.actionProbe?.hitTest === true && timing.actionProbe?.focused === true && timing.actionProbe?.ownerDocument === true)
}
...
export function recomputeFamilyCoverage(familyCoverage) {
  ...
  return Object.values(familyCoverage).every(item => item.realData === true && Array.isArray(item.scenarios) && item.scenarios.length > 0 && item.scenarios.every(scenario => {
    ...
    const componentValid = scenario.component === 'TreeSelect' ? (Array.isArray(source?.matchedKeys) && source.matchedKeys.length >= 5000 && Array.isArray(source?.unmatchedKeys) && source.unmatchedKeys.length > 0 && scenario.logicalSearchMatches === source.matchedKeys.length) : scenario.component === 'Cascader' ? (Array.isArray(source?.paths) && source.paths.length > 10000 && scenario.search?.matchedPaths?.length === 10000 && scenario.flattenedSearchLeaves === 10000 && scenario.lazy?.pending === true && scenario.lazy?.resolved === true && scenario.lazy?.retry === true && scenario.lazy?.cancelled === true && scenario.lazy?.staleIgnored === true) : scenario.component === 'Tree' ? (scenario.label === 'flat-roots-10000' ? source.keys.length === 10000 : scenario.childrenBefore === 100 && scenario.childrenAfter === source.keys.length) : true
