...
test('runs the QG5 production suite in Firefox, desktop WebKit, and mobile WebKit', () => {
  ...
  assert.match(configSource, /const crossBrowserTests = \[qg2Only, qg5Only, qg5R1Only, formEngineOnly, formListOnly, d4IframeOnly, d4SelectionOnly, d4VirtualOnly, d4TreeVirtualOnly, d4TreeSelectVirtualOnly, d4CascaderVirtualOnly, d5TableOnly, d5TableBOnly, d5TableCOnly, d6PickerUploadOnly, d7DndOnly, d8AiOnly\]/)
  ...
  assert.ok(configSource.includes('const formListOnly = /form-list\\.spec\\.ts/'))
...
test('routes the Form.List browser suite through all five configured browsers', () => {
  assert.ok(configSource.includes("const formListOnly = /form-list\\.spec\\.ts/"))
  assert.match(configSource, /const crossBrowserTests = \[[^\]]*formListOnly[^\]]*\]/)
  for (const project of ['desktop', 'mobile', 'desktop-firefox', 'desktop-webkit', 'mobile-webkit']) {
    assert.match(configSource, new RegExp(String.raw`name:\s*'${project}'`))
  }
  for (const project of ['desktop-firefox', 'desktop-webkit', 'mobile-webkit']) {
