...
const checks = { virtualInteractive: summary.virtual.medianInteractiveMs <= 500 && summary.virtual.medianInteractiveMs <= summary['full-dom'].medianInteractiveMs * 0.5, fullDom10k: summary['full-dom'].medianLogicalRows === 10000 && summary['full-dom'].spacerCounts.every(count => count === 0), virtualWindow: summary.virtual.medianLogicalRows > 0 && summary.virtual.medianLogicalRows <= 20 && summary.virtual.spacerCounts.every(count => count === 2), aria10k: results.every(result => result.ariaRowCount === 10000), longTasks: results.every(result => result.longTaskMax <= 100), cls: results.every(result => result.cls <= 0.1), tableGzipDelta: candidate.gzipBytes - baseline.gzipBytes <= 12 * 1024 }
