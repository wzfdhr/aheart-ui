...
try {
  for (let round = 0; round < rounds; round++) for (const [mode, url] of (round % 2 ? [['virtual', virtualUrl], ['full-dom', fullUrl]] : [['full-dom', fullUrl], ['virtual', virtualUrl]])) {
    ...
    await page.addInitScript(() => {
      ...
      new PerformanceObserver(list => { for (const e of list.getEntries()) { const scroller = document.querySelector('[data-aheart-virtual-scroll]'); const sources = e.sources?.map(s => { const n = s.node; const cs = n ? getComputedStyle(n) : null; return { tag: n?.tagName ?? null, id: n?.id ?? null, class: n?.className ?? null, dataset: n ? { ...n.dataset } : null, outerHTML: n?.outerHTML?.slice(0, 500) ?? null, isConnected: n?.isConnected ?? false, display: cs?.display ?? null, position: cs?.position ?? null, height: cs?.height ?? null, previousRect: s.previousRect, currentRect: s.currentRect } }) ?? []; window.__d5c.shiftEntries.push({ startTime: e.startTime, value: e.value, hadRecentInput: e.hadRecentInput, scrollTop: scroller?.scrollTop ?? null, first: document.querySelector('tr[data-table-row]')?.getAttribute('data-aheart-virtual-logical-item') ?? null, entryKeys: [...document.querySelectorAll('tr[data-table-row]')].map(n => n.getAttribute('data-aheart-virtual-logical-item')), sources }); if (!e.hadRecentInput) window.__d5c.cls += e.value } }).observe({ type: 'layout-shift', buffered: true })
    })
