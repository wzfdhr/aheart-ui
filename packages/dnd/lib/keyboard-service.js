  ...
  if (frameElement && ParentObserver) {
    frameObserver = new ParentObserver(() => {
      ...
      if (!frameElement.isConnected) {
        ...
        dndAnnouncer.disposeDndLiveRegion(ownerDocument);
...
function register(registration, kind) {
  ...
  const releaseLiveRegion = dndAnnouncer.acquireDndLiveRegion(ownerDocument);
  ...
  const keydown = (event) => {
    ...
    if (registration.isDisabled()) {
      dndAnnouncer.announceDnd(ownerDocument, kind === "zone" ? "目标已禁用" : "项目已禁用");
      return;
    }
    if (kind === "source") {
      ...
      dndAnnouncer.announceDnd(ownerDocument, `${session2.sourceLabel}，已抓取`);
      return;
    }
    const session = sessions.get(ownerDocument);
    if (!session) {
      dndAnnouncer.announceDnd(ownerDocument, `${registration.getLabel()}，没有可放置的项目`);
      return;
    }
    event.preventDefault();
    ...
    if (reason) {
      dndAnnouncer.announceDnd(ownerDocument, reason);
      return;
    }
    sessions.set(ownerDocument, void 0);
    updateKeyboardState(ownerDocument);
    ...
    dndAnnouncer.announceDnd(ownerDocument, `${session.sourceLabel}，已放置到${registration.getLabel()}`);
  };
  ...
  const focus = () => {
    ...
    const session = sessions.get(ownerDocument);
    if (!session) return;
    dndAnnouncer.announceDnd(ownerDocument, compatible(session, registration) ? `${registration.getLabel()}，可放置` : `${registration.getLabel()}，类型不匹配`);
  };
...
exports.cancelKeyboardScope = cancelKeyboardScope;
exports.registerKeyboardSource = registerKeyboardSource;
exports.registerKeyboardZone = registerKeyboardZone;
