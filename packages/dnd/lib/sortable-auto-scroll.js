...
function findScrollableAncestors(element2) {
  const ownerWindow = element2.ownerDocument.defaultView;
  const ancestors = [];
...
function retainElementRegistration(ancestor) {
  ...
  else registrations.set(ancestor, { count: 1, cleanup: element.autoScrollForElements({ element: ancestor }) });
  return () => {
...
function retainMainWindow(ownerDocument) {
  ...
  if (existing) existing.count += 1;
  else windowRegistrations.set(ownerDocument, { count: 1, cleanup: element.autoScrollWindowForElements() });
  return () => {
...
function retainOwnerWindow(element2) {
  const ownerDocument = element2.ownerDocument;
  const ownerWindow = ownerDocument.defaultView;
  if (!ownerWindow) return () => void 0;
  const ancestors = findScrollableAncestors(element2);
  ...
  const scroll = () => {
    var _a, _b;
    raf = 0;
    if (!dragState.isDragActiveFor(ownerDocument)) return;
    const edge = 36;
    let consumed = false;
    ...
    if (dragState.isDragActiveFor(ownerDocument) && consumed && edgeRequested && !raf) raf = ((_b = ownerWindow.requestAnimationFrame) == null ? void 0 : _b.call(ownerWindow, scroll)) ?? ownerWindow.setTimeout(scroll, 16);
  };
  const schedule = (event) => {
    ...
    if (!dragState.isDragActiveFor(ownerDocument) || raf) return;
...
function registerSortableAutoScroll(element2) {
  ...
  const releases = ownerWindow && ownerWindow === globalWindow ? findScrollableAncestors(element2).map(retainElementRegistration) : [];
  releases.push(ownerWindow && ownerWindow === globalWindow ? retainMainWindow(ownerDocument) : retainOwnerWindow(element2));
  let released = false;
