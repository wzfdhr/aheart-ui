...
const _sfc_main = /* @__PURE__ */ vue.defineComponent({
  ...
  setup(__props, { emit: __emit }) {
    ...
    const radioClickHandled = vue.ref(false);
    ...
    const handleTableClickCapture = (event) => {
      ...
      if (focusRow) {
        ...
        const index22 = Number(focusRow.dataset.aheartVirtualLogicalItem);
        const logical2 = Number.isFinite(index22) ? pagedRows.value[index22] : void 0;
        if (logical2)
      ...
      const logical = Number.isFinite(index2) ? pagedRows.value[index2] : void 0;
      if (logical)
    ...
    const narrowLeftConstrained = vue.computed(() => {
    ...
    const usedWidth = (item) => {
      var _a;
      if (narrowLeftConstrained.value && item.utility && item.width)
        return Number.parseFloat(item.width) || 0;
    ...
    const tableNarrowStyle = vue.computed(() => narrowLeftConstrained.value ? { "--aheart-table-narrow-utility-width": narrowUtilityWidth.value } : void 0);
    ...
    const virtualFallbackReason = vue.computed(() => {
      if (props.virtual === false || props.virtual === void 0)
        return "";
        ...
        observedVirtualRows.forEach((row) => {
          if (!rows.has(row)) {
            ...
            if (row.matches("tr[data-table-expanded-row]")) {
              const index2 = Number(row.dataset.aheartVirtualExpandedItem);
              if (Number.isFinite(index2))
                virtualController.clearMeasured(index2, "expanded");
            }
          }
        });
        rows.forEach((row) => {
      ...
      virtualResizeObserver = new Constructor((entries) => {
        ...
        const flush = () => {
          ...
          observedVirtualHeights.forEach((height, target) => {
            ...
            const index2 = Number(element.dataset.aheartVirtualLogicalItem ?? element.dataset.aheartVirtualExpandedItem);
            if (!Number.isFinite(index2))
              return;
            const part = element.matches("tr[data-table-expanded-row]") ? "expanded" : "base";
            const parts = heights.get(index2) ?? /* @__PURE__ */ new Map();
            parts.set(part, height);
            heights.set(index2, parts);
          });
    ...
    const handleRowKeydown = (event, key) => {
      ...
      const findCandidate = (from) => {
        for (let index2 = from; index2 >= 0 && index2 < pagedRows.value.length; index2 += direction) {
          const candidate2 = pagedRows.value[index2];
    ...
    return (_ctx, _cache) => {
      return vue.openBlock(), vue.createElementBlock("section", {
        ...
        vue.createElementVNode("div", {
          ...
          onPointerdownCapture: _cache[2] || (_cache[2] = ($event) => vue.isRef(pointerInteractionPending) ? pointerInteractionPending.value = true : pointerInteractionPending = true),
                    ...
                    return vue.openBlock(), vue.createElementBlock("th", {
                      ...
                      vue.createElementVNode("div", _hoisted_15, [
                        ...
                        column.filterDropdown ? (vue.openBlock(), vue.createElementBlock("button", {
                          ...
                          _cache[4] || (_cache[4] = vue.createElementVNode("span", { "aria-hidden": "true" }, "⌄", -1)),
                ...
                (vue.openBlock(true), vue.createElementBlock(vue.Fragment, null, vue.renderList(virtualRenderEntries.value, (entry) => {
                  return vue.openBlock(), vue.createElementBlock(vue.Fragment, {
                    ...
                    entry.kind === "gap" ? (vue.openBlock(), vue.createElementBlock("tr", {
                      ...
                      ], 40, _hoisted_29)) : vue.createCommentVNode("", true)
                    ], 64))
                  ], 64);
                }), 128)),
        ...
        _ctx.loading ? (vue.openBlock(), vue.createElementBlock("div", _hoisted_33, [
