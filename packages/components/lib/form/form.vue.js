...
const _sfc_main = /* @__PURE__ */ vue.defineComponent({
  ...
  setup(__props, { expose: __expose, emit: __emit }) {
    ...
    const getRules = (name) => {
      var _a;
      return [...typeof name === "string" ? props.rules[name] ?? [] : [], ...((_a = fieldStates[namePath.namePathKey(name)]) == null ? void 0 : _a.rules) ?? []];
    };
    ...
    const getDefaultMessage = (name, rule) => {
      const label = namePath.namePathLabel(name);
      if (rule.required) {
    ...
    const getRuleMessageVariables = (name, rule) => {
      var _a;
      return {
        name: namePath.namePathLabel(name),
        ...((_a = fieldStates[namePath.namePathKey(name)]) == null ? void 0 : _a.messageVariables) ?? {},
    ...
    const collectRuleErrors = (name, rules, validateFirst) => {
      if (validateFirst === true) {
        const runNext = (index) => {
          for (let ruleIndex = index; ruleIndex < rules.length; ruleIndex += 1) {
            const result = validateRule(name, namePath.getNamePathValue(props.model, name), rules[ruleIndex]);
            if (isPromiseLike(result)) {
      ...
      const results = rules.map((rule) => validateRule(name, namePath.getNamePathValue(props.model, name), rule));
    ...
    const validateFields = (names) => {
      ...
      const expectedRuns = targets.map((name) => validationRuns.get(namePath.namePathKey(name)));
      const finish = (resolved) => {
        const outOfDate = disposed || startRevision !== validationRevision || resolved.includes(staleValidation) || targets.some((name, index) => validationRuns.get(namePath.namePathKey(name)) !== expectedRuns[index]);
        return {
          values: cloneValues(),
    ...
    const setFieldValue = (name, value) => {
      namePath.setNamePathValue(props.model, name, value);
      observeModel();
      clearValidate([name]);
    };
    ...
    const getFieldValue = (name) => namePath.getNamePathValue(props.model, name);
    const pickValues = (names) => names.reduce((values, name) => {
      if (typeof name === "string")
        values[name] = namePath.getNamePathValue(props.model, name);
      else
        namePath.setNamePathValue(values, name, namePath.getNamePathValue(props.model, name));
      return values;
    }, {});
    ...
    const getFieldError = (name) => {
      var _a;
      return [...((_a = fieldStates[namePath.namePathKey(name)]) == null ? void 0 : _a.errors) ?? [], ...externalErrors.get(namePath.namePathKey(name)) ?? []];
    };
    ...
    const queueValidation = (name, event) => {
      ...
      void vue.nextTick(() => {
        ...
        for (const task of batch) {
          if (!retiredFieldNames.has(namePath.namePathKey(task.name)))
            void validateField(task.name, task.event);
        }
      });
    };
    const clearExternalError = (name) => externalErrors.delete(namePath.namePathKey(name));
    ...
    vue.watch(() => [props.validateTrigger, props.preserve], () => {
