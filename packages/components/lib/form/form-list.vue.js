...
const _sfc_main = /* @__PURE__ */ vue.defineComponent({
  ...{ name: "AFormList" },
  __name: "form-list",
  props: types.formListProps,
  setup(__props) {
    const props = __props;
    const form = vue.inject(internalContext.formInternalContextKey, void 0);
    const parentList = vue.inject(listContext.formListNameContextKey, void 0);
    const instanceId = useStableId.useStableId(void 0, "aheart-form-list").value;
    const fullName = vue.computed(() => (parentList == null ? void 0 : parentList.resolveName(props.name)) ?? namePath.normalizeNamePath(props.name));
    const owner = vue.computed(() => `${(parentList == null ? void 0 : parentList.resolveOwner(props.name)) ?? "form-root"}/list:${instanceId}`);
    ...
    vue.watch(
      ...
      (value) => {
        if (value !== void 0 && !Array.isArray(value) && value !== warnedInvalidValue) {
          ...
          devWarn(`model path ${namePath.namePathKey(fullName.value)} must contain an array`);
        }
      },
      { immediate: true }
    );
    ...
    const fields = vue.computed(() => currentItems.value.map((_item, index) => ({
      key: tokens.value[index] ?? `${instanceId}-pending-${index}`,
      fieldKey: tokens.value[index] ?? `${instanceId}-pending-${index}`,
      name: index
    })));
    const errors = vue.computed(() => (form == null ? void 0 : form.getFieldErrors(fullName.value)) ?? []);
    ...
    vue.provide(listContext.formListNameContextKey, {
      prefix: fullName,
      resolveName: (name) => namePath.resolveNamePath(fullName.value, name),
      resolveOwner: (name) => {
        const segments = namePath.namePathSegments(name);
        ...
        return token ? `${owner.value}/item:${token}/field:${JSON.stringify(tail)}` : `${owner.value}/field:${namePath.namePathKey(name)}`;
      }
    });
    vue.onBeforeUnmount(() => {
      if (registeredName !== void 0 && registeredOwner !== void 0) {
