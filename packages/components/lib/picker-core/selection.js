...
const normalizeRangeValue = (value, format, order = true, allowEmpty = [false, false]) => {
  ...
  if (!start && !allowEmpty[0] || !end && !allowEmpty[1])
    return void 0;
  if (order && start && end && codec.comparePickerValues(start, end, format) > 0)
    return [end, start];
  return [start, end];
};
...
exports.advanceRangeSelection = advanceRangeSelection;
exports.normalizeMultipleValues = normalizeMultipleValues;
exports.normalizeRangeValue = normalizeRangeValue;
exports.updateRangeDraft = updateRangeDraft;
