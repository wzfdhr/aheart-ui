...
function useTreeVirtual(root, config, nodes, focusedKey, disabled) {
  ...
  const actualFocusedKey = vue.ref();
  const focusRecoveryKey = vue.ref();
  ...
  const tokenFor = (key) => treeIndex.treeKeyToken(key);
  const pinnedIndexes = vue.computed(() => [focusedKey.value, actualFocusedKey.value, pendingKey.value].map(indexFor).filter((index) => index >= 0));
  ...
  vue.watch(config, (next, previous) => {
    const element = root.value;
  ...
  vue.watch(() => nodes.value.map((node) => tokenFor(node.key)), (keys) => {
