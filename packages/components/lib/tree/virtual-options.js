...
function normalizeTreeVirtual(value, warn) {
  ...
  if (value === true)
    return { ...DEFAULTS };
  if (!plain(value)) {
    warn == null ? void 0 : warn("[ATree] invalid virtual options; virtualization is disabled.");
    return null;
  }
  const source = value;
  ...
  if (source.height !== void 0) {
    if (positive(source.height))
      ...
      warn == null ? void 0 : warn("[ATree] virtual.height must be a positive number; using the default.");
  }
  if (source.estimateSize !== void 0) {
    if (positive(source.estimateSize))
      result.estimateSize = source.estimateSize;
    else
      warn == null ? void 0 : warn("[ATree] virtual.estimateSize must be a positive number; using the default.");
  }
  if (source.overscan !== void 0) {
    if (typeof source.overscan === "number" && Number.isSafeInteger(source.overscan) && source.overscan >= 0)
      ...
      warn == null ? void 0 : warn("[ATree] virtual.overscan must be a non-negative safe integer; using the default.");
  }
  return result;
}
