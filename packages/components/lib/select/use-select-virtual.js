...
function useSelectVirtual(input) {
  const config = vue.computed(() => virtualOptions.normalizeSelectVirtual(input.config(), void 0));
  const enabled = vue.computed(() => Boolean(config.value && input.open.value && !input.disabled.value));
  const geometry = vue.ref({ top: 4, bottom: 4, gap: 2 });
  let alive = true;
  let scrollVersion = 0;
  let activeScroll;
  ...
  vue.watch([input.activeKey, input.activeIndex, input.open, input.popup, config], () => {
    void scrollToActive();
  }, { flush: "post" });
  vue.watch(input.popup, (popup, _previous, cleanup) => {
    if (!popup)
      return;
...
exports.useSelectVirtual = useSelectVirtual;
