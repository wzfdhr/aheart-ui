...
export const zhCN: AheartLocale = {
  ...
  upload: {
    ...
    remove: (name) => `移除 ${name}`, cancel: (name) => `取消上传 ${name}`, retry: (name) => `重试 ${name}`
  }
}
...
export const enUS: AheartLocale = {
  ...
  upload: {
    selectFile: 'Select file', upload: 'Upload', done: 'Done', failed: 'Failed', cancelled: 'Cancelled',
    validationFailed: 'Validation failed', timeout: 'Upload timed out', removeAction: 'Remove', cancelAction: 'Cancel', retryAction: 'Retry',
    remove: (name) => `Remove ${name}`, cancel: (name) => `Cancel upload ${name}`, retry: (name) => `Retry ${name}`
  }
}
  ...
  const mergedConfig = computed<AheartConfig>(() => {
    ...
    return {
      ...
      locale: {
        ...
        upload: {
          ...zhCN.upload,
          ...parent.locale?.upload,
          ...current.locale?.upload
        }
      },
      theme: {
        ...parent.theme,
