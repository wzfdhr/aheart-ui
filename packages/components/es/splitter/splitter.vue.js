...
const _sfc_main = defineComponent({
  ...
  setup(props, { emit, slots }) {
    ...
    const innerSizes = ref([...props.defaultSizes]);
    const pendingSizes = ref(null);
    const collapsedPanels = ref(/* @__PURE__ */ new Set());
    const collapsedSizes = /* @__PURE__ */ new Map();
    const dragState = ref(null);
    let resizeObserver;
    const panelNodes = computed(() => {
      var _a;
      return (((_a = slots.default) == null ? void 0 : _a.call(slots)) ?? []).filter((node) => node.type !== Comment && node.type !== Text);
    });
    ...
    const resolvedSizes = computed(
      () => pendingSizes.value ?? resolveSplitterSizes({
    ...
    const togglePanelCollapse = (handleIndex) => {
      ...
      const targetSize = isCollapsed ? collapsedSizes.get(panelIndex) ?? currentSizes[panelIndex] : resolveSplitterPanelBounds(panelConstraints.value[panelIndex], solverContainerSize.value).min;
    ...
    return () => {
      ...
      panelNodes.value.forEach((panel, index) => {
        var _a;
        children.push(cloneVNode(panel, { style: [(_a = panel.props) == null ? void 0 : _a.style, panelStyle(index)] }));
