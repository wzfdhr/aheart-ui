  ...
  setup(__props, { expose: __expose, emit: __emit }) {
    ...
    const resetFields = (names) => {
      ...
      try {
        ...
        const controllers = Array.from(new Set(listControllers.values())).filter((controller) => targetKeys.has(namePathKey(controller.name))).sort((left, right) => namePathSegments(left.name).length - namePathSegments(right.name).length);
        for (const controller of controllers) {
          if (listControllers.get(namePathKey(controller.name)) !== controller)
            continue;
    ...
    const remapOwnedListState = (listName, oldIndexToNewIndex, ownerPrefix) => {
      ...
      const fieldSnapshots = Array.from(fieldNames.entries()).flatMap(([oldKey, name]) => {
        ...
        const match = owner && isOwnedBy(owner, ownerPrefix) ? matchListDescendant(name, listName) : void 0;
        if (!match)
          return [];
      ...
      const nestedControllers = Array.from(new Set(listControllers.values())).flatMap((controller) => {
        const match = isOwnedBy(controller.owner, ownerPrefix) ? matchListDescendant(controller.name, listName) : void 0;
        if (!match)
          return [];
      ...
      for (const snapshot of fieldSnapshots) {
        ...
        const nextIndex = (_a = matchListDescendant(snapshot.nextName, listName)) == null ? void 0 : _a.index;
        ...
        snapshot.state.dependencies = snapshot.state.dependencies.map(
          (dependency) => remapListDescendant(dependency, listName, oldIndexToNewIndex) ?? dependency
        );
        ...
        notifiedValues.set(nextKey, snapshot.hasNotified ? cloneInitialValue(snapshot.notified) : cloneInitialValue(getNamePathValue(props.model, snapshot.nextName)));
        if (snapshot.owner)
    ...
    const reconcileExternalLists = () => {
      const skippedValueInvalidation = /* @__PURE__ */ new Set();
      const controllers = Array.from(new Set(listControllers.values())).sort((left, right) => namePathSegments(left.name).length - namePathSegments(right.name).length);
      for (const controller of controllers) {
        if (listControllers.get(namePathKey(controller.name)) !== controller)
          continue;
        const value = getNamePathValue(props.model, controller.name);
