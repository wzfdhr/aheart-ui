...
test('Message workbench exercises real service entry, updates, stacking, destroy, and reset', async ({ page }) => {
  ...
  await expect(page.locator('.aheart-message-notice')).toHaveCount(0)

  await workbench.getByRole('button', { name: 'Configured message', exact: true }).click()
  await expect(page.locator('.aheart-message').last()).toHaveCSS('top', '32px')
  await workbench.getByRole('button', { name: 'Reset config', exact: true }).click()
  await expect(page.locator('.aheart-message').last()).toHaveCSS('top', '8px')
  await expectNoRuntimeErrors(page)
})
...
test('Modal workbench covers async success, failure recovery, Escape focus, and controlled close rejection', async ({ page }) => {
  ...
  await expect(dialog.getByRole('button', { name: 'Confirm', exact: true })).toHaveAttribute('aria-busy', 'true')
  ...
  await failedDialog.getByRole('button', { name: 'Confirm', exact: true }).click()
  ...
  await expect(failedDialog.getByRole('button', { name: 'Confirm', exact: true })).toBeEnabled()
  await expect(failedDialog.getByRole('button', { name: 'Confirm', exact: true })).not.toHaveAttribute('aria-busy', 'true')
