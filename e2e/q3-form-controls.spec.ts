...
test('TimePicker defaults to one-minute steps and supports configured steps and confirmation', async ({ page }) => {
  ...
  await expect(page.locator('.aheart-time-picker__panel')).toHaveCount(0)

  const steppedInput = page.locator('.aheart-demo-panel').nth(1).getByRole('combobox')
  await steppedInput.focus()
  const steppedPanel = page.locator('.aheart-time-picker__panel:visible')
  await expect(steppedPanel.getByRole('listbox', { name: '分' }).getByRole('option')).toHaveCount(12)
  await expect(steppedPanel.getByRole('listbox', { name: '秒' }).getByRole('option')).toHaveCount(6)
  await page.keyboard.press('Escape')
  await expect(page.locator('.aheart-time-picker__panel')).toHaveCount(0)

  const confirmInput = page.locator('.aheart-demo-panel').nth(2).getByRole('combobox')
  await confirmInput.focus()
  const confirmPanel = page.locator('.aheart-time-picker__panel:visible')
  await confirmPanel.getByRole('listbox', { name: '分' }).getByRole('option', { name: '31', exact: true }).click()
  await expect(confirmInput).toHaveValue('14:31:00')
  await confirmPanel.getByRole('button', { name: '确定', exact: true }).click()
  await expect(confirmInput).toHaveValue('14:31:00')
})
