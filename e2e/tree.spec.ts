...
test('Tree workbench keeps controlled keys, focus, keyboard navigation, and disabled states observable', async ({ page }) => {
  ...
  await directNode(tree.getByRole('treeitem').first()).press('End')
  await expect(directNode(tree.getByRole('treeitem').last())).toBeFocused()

  const checkedLeaf = leaf.getByRole('checkbox')
  await checkedLeaf.click()
  ...
  await directNode(anotherLeaf).press('Enter')
