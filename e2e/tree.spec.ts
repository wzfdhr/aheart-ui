...
test('Tree workbench keeps controlled keys, focus, keyboard navigation, and disabled states observable', async ({ page }) => {
  ...
  const root = tree.getByRole('treeitem', { name: /根节点/ }).first()
  const leaf = tree.getByRole('treeitem', { name: /可选叶节点/ }).first()
  ...
  await root.press('ArrowRight')
  await expect(tree.getByRole('treeitem', { name: /一级子节点/ }).first()).toBeFocused()

  await tree.getByRole('treeitem', { name: /一级子节点/ }).first().press('ArrowRight')
  await expect(tree.getByRole('treeitem', { name: /二级子节点/ }).first()).toBeFocused()
  await tree.getByRole('treeitem', { name: /二级子节点/ }).first().press('ArrowLeft')
  await expect(tree.getByRole('treeitem', { name: /一级子节点/ }).first()).toBeFocused()

  await tree.getByRole('treeitem', { name: /二级子节点/ }).first().focus()
  await tree.getByRole('treeitem', { name: /二级子节点/ }).first().press('Home')
  ...
  await leaf.press('Enter')
  await tree.getByRole('treeitem', { name: /另一个叶节点/ }).first().press('Enter')
  ...
  const disabledLeaf = tree.getByRole('treeitem', { name: /禁用叶节点/ }).first()
