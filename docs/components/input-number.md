...
除组件已声明的属性外，原生 `<input>` 属性与监听器会透传到内部输入框，例如 `name`、`autocomplete`、`pattern`、`aria-*`、`@blur`；组件上的 `class`、`style` 和鼠标事件（如 `@click`、`@mousedown`、`@mousemove`）保留在根节点，根节点 `mousedown` 会聚焦内部输入框。`@input` 为组件事件，返回当前原始输入文本。
