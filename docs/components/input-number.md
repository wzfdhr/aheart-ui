...
未显式传入 `precision` 时，默认显示精度会从当前值和 `step` 的小数位数中自动推导。
有效步进后输入框会保持焦点，便于继续键盘输入或方向键调整。
当当前值达到 `max` 或 `min` 时，对应方向的步进会被禁用并忽略。
...
输入框聚焦后，滚轮会累计移动距离，达到步进阈值后触发一次变化。长按上下控制按钮会连续步进。键盘方向键步进支持 `ArrowUp`/`ArrowDown` 和 `Up`/`Down` key 值；按住 <kbd>Shift</kbd> 会使用 10 倍步长。输入法组合输入期间不会触发方向键步进，也不会提前解析提交输入值，组合结束后再解析当前输入。
...
除组件已声明的属性外，原生 `<input>` 属性与监听器会透传到内部输入框，例如 `name`、`autocomplete`、`pattern`、`aria-*`、`@blur`；内部输入框默认 `autocomplete="off"`，显式传入时可覆盖；内部输入框会提供 `spinbutton` ARIA 语义和值范围元数据，并且仅在当前值为有效数值时输出 `aria-valuenow`；组件上的 `class`、`style` 和鼠标事件（如 `@click`、`@mousedown`、`@mousemove`）保留在根节点，内部输入框聚焦时根节点会带有 `is-focused` 状态类，当前值为非空无效数值时根节点会带有 `is-not-a-number` 状态类，当前有效值超出 `min`/`max` 时根节点会带有 `is-out-of-range` 状态类，步进控制按钮只会在当前有效值达到 `min`/`max` 边界时禁用，根节点 `mousedown` 会聚焦内部输入框并阻止默认 mouse-down 行为。`@input` 为组件事件，返回当前原始输入文本。
...
| step | 控制按钮、键盘或滚轮步进时触发，`stringMode` 下 value 返回字符串；`info.offset` 保留当前步长，方向由 `info.type` 表示 | `(value: number \| string, info: { offset: number \| string; type: 'up' \| 'down'; emitter: 'handler' \| 'keyboard' \| 'wheel' }) => void` |
