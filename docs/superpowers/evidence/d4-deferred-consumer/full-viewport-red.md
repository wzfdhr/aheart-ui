...
The first page-recycling candidate then failed immediately with an explicit `measureCase Tree/1000/fixed/full` context because a newly created Page reports `about:blank`; deriving the origin from it produced `null/?component=...`. The repair now passes the already verified production-preview `base` URL into every Chromium, Firefox, and WebKit measurement. The RED test rejects any recycled-page call that omits that explicit base.

After the explicit-origin repair, the run again completed 264 checkpoints and stopped at the same `Cascader/1000/dynamic/full` readiness boundary even though every matrix case used a fresh Page. Together with the isolated fresh-Browser success, this shows the retained state belongs to the long-lived browser process. The next repair restarts the browser process between Tree, TreeSelect, and Cascader families for all three engines while preserving serial execution, exact timings, observer ledgers, and cleanup counters.

Restarting only at component-family boundaries still reproduced the same timeout after Cascader fixed/coarse completed in the new process. The final isolation rule therefore restarts the browser after every `component/count/rowMode` case (twelve navigations: two warm-ups plus ten alternating measured paths). The 30-second readiness limit and the measured page logic remain unchanged.
