...
Follow-up independent review found that nonempty `export {};` with recomputed hashes still passed. The new isolated negative test reproduced `Missing expected exception`. The validator now parses emitted JavaScript and requires all three named exports in an entry; 94 contract tests pass. This does not by itself prove export implementation authenticity; real isolated build/artifact evidence and the corrected quantitative measurement remain outstanding.
