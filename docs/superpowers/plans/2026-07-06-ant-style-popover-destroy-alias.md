# Ant Style Popover Destroy Alias Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add the deprecated `destroyTooltipOnHide` compatibility alias to Popover.

**Architecture:** Keep Popover's DOM ownership and hidden lifecycle unchanged. Add the alias prop in `types.ts`, derive a shared `shouldDestroyOnHidden` computed in `popover.vue`, and route the existing popup render condition through that computed value.

**Tech Stack:** Vue 3, TypeScript, Vitest, Vue Test Utils, Vite component build, VitePress docs.

---

### Task 1: Failing Popover Destroy Alias Test

**Files:**
- Modify: `packages/components/src/popover/__tests__/popover.test.ts`

- [x] Add a test that opens and closes a popover with `destroyTooltipOnHide`.
...
- [x] Add `destroyTooltipOnHide` to `popoverProps`.
...
- [x] Add a Popover API row for `destroyTooltipOnHide`.
