...
The next two verify jobs exposed platform-specific raw gzip/tar bytes for the repository-rebuilt baseline while all ten QG5 jobs passed. The content-ledger repair keeps exact commit provenance, validates all 995 extracted files against a pinned deterministic fingerprint and still binds each real tgz raw hash in its own manifest. Its locale-dependent first attempt was rejected; the stable-order guard and integration 49/49 passed. This remains part of the 233-script suite and requires a new exact-head CI result.

After the content ledger passed remotely, both verify jobs exposed a Node 20.20.2 versus approved 24.17.0 drift; their ten QG5 siblings passed. The workflow now pins 24.17.0 in verify and QG5, with a genuine 0/2 to 2/2 guard transition. The approved runtime contract was preserved. Exact-head CI must still prove the repair.
