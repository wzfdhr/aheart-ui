...
最终10k打开后的DOM：全量三轮均10000，虚拟固定均12、动态均10。Long Task数量/总时长的三轮中位：全量固定冷开2项/246ms、热开2项/207ms；全量动态冷开2项/270ms、热开2项/227ms；虚拟对应四组均0项。搜索和20键区间两模式均未捕获Long Task，不因壁钟时间较大就伪造长任务归因；这也不表示未来任意内容/设备都没有长任务。
...
最终独立复验现已完成：前后指纹均1f7b2f22…505bd9；components1121、DnD44、AI66、scripts86、三包类型、确定性双构建、pack971/71/111通过；完整E2E576条中449通过/127既有平台跳过/0失败，splitter remount亦通过。最终consumer36场景、4组hydrate、CJS、emptyVisible及稳定字节/模块集一致。详见[最终独立测试续验](2026-09-07-d4-c-test-review.md)。独立测试结束后已重新采集3张最终运行图并审核；当前仅待产品经理最终裁定，未PR/推送/合并/部署或启动D5。
