...
尚无独立专项证据的维度包括：运行时字体变化、宽度变化引起的重新换行、中部条目自身增删/重排后的锚点、完整tags与multiple/clear组合矩阵，以及所有hover变体。PageUp/PageDown并非当前Select既有快捷键，不能当作已实现；是否新增由产品定范围。物理触控设备与读屏未验证，只披露证据限制，不新增为本批或Draft PR的门槛。C产品验收不能扩写为“全部边界无缺口”；这些维度供后续影响范围/完整D4范围核对。
...
2. **原始 D4 泛化虚拟化 checkbox 仍待范围裁定。** Select 子项已实现并验收，但原始第 13 项包含更宽的 API/依赖/方案与其他组件边界，不能只凭 Select 完成就将总项改成 `[x]`。
3. **交付门禁未闭合。** A/B/C 产品报告均不等于远端 Draft PR CI、主线 CI/Pages、合并或发布；这些门禁仍需按计划执行。
