...
The extended maintained recovery suite now has14 cases, **9 passed / 5 failed**, proving all four remaining P2 root causes (mode gating has two scenarios). The strengthened zero case reads the actual nested Tree prop `{ height:0, estimateSize:28, overscan:4 }`, requires a positive public height, preserves internal CSS0, and separately preserves public invalid-input warnings/fallback. Fresh same-run source-before/source-after hashes match. Evidence: `/tmp/d4-treeselect-rerecovery-prop-independent-20260910/`. This is the RED baseline for the next four-item repair, not acceptance.

## Repaired source freeze and expanded browser evidence

Source commit `effc110233aef976a3a822a92376a3d09b587e26` closed the four findings in a fresh independent17/17 developer run (P0/P1/P2=`0/0/0` for that reviewed scope). Same-run source hashes matched and the source diff against that commit was empty. Report: `/tmp/d4-treeselect-effc110-review.UceJPZ/review.md`.

Independent119/119 Tree/TreeSelect tests and public types passed with fresh source snapshots. The strengthened seven-scenario/five-browser matrix passed35/35 at `049bdd5`; it waits for stable popup opacity, checks actual font/row geometry and every scroll offset's window coverage, and no longer misuses glyph scroll metrics as a clipping assertion. Evidence: `/tmp/d4-treeselect-final-cNuttn/` and `/tmp/d4-treeselect-browser-final-rHgRPY/`.

Full repository tests passed1663 (components1292, DnD79, AI200, scripts92), as did root typecheck, three-package build, docs build and release pack1015/79/115. Source stayed identical to `effc110`. Log: `/tmp/d4-treeselect-release.5ibDk8/run.log`. Generated artifacts and public documentation were saved at `4f63eb1`; this did not grant product acceptance.

## New real-Chromium popup retry focus blocker

The added **real popup** lazy scenario at `e649229` reached successful children but lost root focus in desktop/mobile Chromium. Targeted results were3/5 and the expanded TreeSelect matrix38/40; Firefox and WebKit passed that scenario, and the independent standalone Tree regression passed45/45. Source/fixture/spec hashes matched. Evidence: `/tmp/d4-treeselect-lazy-final-21Vwjk/`.

This is different from the earlier Teleport-stub failure: the real Tree uid, Tree DOM and root DOM stay unchanged, expansion/data succeed, and only actual focus ends on BODY. Independent unpaused Chromium reproduction and a read-only CDP breakpoint show the exact cancellation stack: TreeSelect parent focusout nextTick → bridge.cancel → Tree cancelPending, while `pendingKey`/`handoffKey` are the lazy root and handoffReady is true. Chromium reports null relatedTarget while removing the focused retry control; the parent mistakes that structural handoff for user departure.

Root-cause evidence: `/tmp/d4-treeselect-chromium-retry.x293B1/` (`root-cause.md`, `retry.log`, `retry-breakpoints-final.log`, executable browser diagnostic). The minimal repair belongs only to the parent focusout decision; it must preserve Tree's valid removal handoff while retaining connected explicit blur, concrete outside focus and close/query cancellation. No loader/expansion workaround is permitted. A maintained Chrome-style removal-event unit and explicit-departure controls precede the source repair. TreeSelect acceptance is reopened for this focus integration blocker; previous green scopes are not final approval.

## Parent focus repair and contrast repair

Commit `91f212b` limited the parent focusout exception to a detached Tree-origin control, null/body related target, owner-document BODY focus and still-accepted open state. Connected blur, concrete outside focus and close/query cancellation remain active. The independent final focus freeze passed124/124 maintained unit tests, public types, TreeSelect40/40 and Tree45/45; evidence is `/tmp/d4-treeselect-final-freeze-G2C1rS/`.

The screenshot-first audit then measured ordinary-text contrast below4.5:1 for the retry text in normal, hover and selected rows and for the selected title. The corrected five-browser RED was0/5 with ratios3.2677,3.1307,2.9175 and3.6641 in `/tmp/d4-treeselect-contrast-complete-kxVG8a/matrix-5-corrected.log`. Commit `6ecf8cb` changed only the Tree-local selected-title and retry foreground declarations. Independent five-browser GREEN measured5.3056,5.0831,4.7368 and4.9447; the TreeSelect45/45, Tree45/45, unit124/124 and public-type checks passed in `/tmp/d4-treeselect-candidate-25iYY7/`.

## Screenshot-discovered collapsed retry repair loop

The final-state audit exercised a state absent from the first browser case: after a lazy root fails, selecting it closes the single-select popup; reopening leaves the selected root collapsed while its retry action remains visible and enabled. Keyboard retry entered `state=aborted; attempts=2; aborts=1` in all five projects. The real RED is commit `66dc6ac` with0/5 evidence in `/tmp/d4-treeselect-lazy-reopen-fIgLtb/`. Hiding or disabling the still-visible retry was rejected as a product behavior change.

The first repair `53fdd5d` expanded before retry, but independent review found same-turn unmount, accepted close, disabled focus and explicit-blur races. Maintained RED commits `1e2435c`, `2710a66` and `c4a5634` froze those paths. The second repair `c759ed4` closed the lifecycle races but left terminal retry intents and two owner-document focus listeners; listener RED was committed at `d41ace2`. The third repair `ab5b9f0` introduced ownership-aware retirement, then independent review found that an older same-key Home callback could retire a newer retry generation. Commit `f199050` preserved that ownership RED. Each rejected repair stayed outside product acceptance.

Final source commit `ad18e3b089a7f9ab0b1de35f2c09ea3395d01c28` makes `focusNode` receive the exact retry intent it owns. Terminal paths retire only that object and only cancel its still-current generation; ordinary Home/End/search navigation never captures it. This closes controlled refusal, node removal, disabled, close, unmount, blur, attempt exhaustion, nonvirtual/fallback and old-versus-new navigation boundaries without aborting an already-started loader.

Independent developer review passed the repository20/20 and expanded ownership24/24 diagnostics, P0/P1/P2=`0/0/0`, in `/tmp/d4-tree-retry-ad18e3b.KQ2pR1/`. Independent functional verification then passed Tree lazy plus retry30/30, the Tree/TreeSelect maintenance set142/142, public types, TreeSelect50/50 and Tree45/45 with stable before/after hashes and zero console error, page error or skip; evidence is `/tmp/d4-ad18-final-DkTQXH/`.

The current functional candidate is `ad18e3b`. It is cleared for TreeSelect design, independent test-manager and product-manager gates. Generated outputs, full-repository rebuild/release pack and combined Tree/TreeSelect/Cascader consumer/performance/delivery remain later gates and are not claimed here.
