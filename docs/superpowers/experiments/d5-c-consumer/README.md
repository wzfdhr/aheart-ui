...
Reproduce from the repository root with `corepack pnpm --filter ./packages/components pack --pack-destination /tmp/d5-c-tgz`, then `node docs/superpowers/experiments/d5-c-consumer/run.mjs --tarball /tmp/d5-c-tgz/aheart-ui-1.0.0.tgz --out /tmp/d5-c-consumer.json`. The runner copies `types.ts` and `tsconfig.json` into its isolated root, installs the real tgz, runs `tsc`, and records JSON when `--out` is provided; no fixture-directory install is required.
