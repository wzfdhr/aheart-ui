...
For the frozen review candidate, pass `--new-lockfile docs/superpowers/evidence/d4-c/consumer/new-lock.json` and `--old-lockfile docs/superpowers/evidence/d4-c/consumer/old-lock.json`; npm then runs only clean install against those locks. The preserved old tarball is `docs/superpowers/evidence/d4-c/consumer/baseline.tgz` (A candidate `829531b`). The new tarball must be repacked from the reviewed generated outputs and match the new lock integrity. `--include=dev` is explicit because Vite sets NODE_ENV=production in its process and the subsequent old-package probe still needs its evaluation tools. This does not install those tools in the core package.
