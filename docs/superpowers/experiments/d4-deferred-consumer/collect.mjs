...
async function collectSmoke(temporary) {
  ...
  try {
    for (let mask = 0; mask < 8; mask++) {
      ...
      const hydratedState = await page.evaluate(async () => { const app = document.querySelector('#app'); const before = app?.innerHTML ?? ''; const beforeBody = document.body.innerHTML; const ids = String(document.querySelectorAll('[id^="d4-"]').length); window.__d4EventLog = []; const treeTarget = document.querySelector('#d4-tree .aheart-tree__switcher'); const treeSelectTarget = document.querySelector('#d4-tree-select input[type="checkbox"], #d4-tree-select .aheart-tree-select__trigger'); const cascaderTarget = document.querySelector('#d4-cascader .aheart-cascader__option, #d4-cascader .aheart-cascader__trigger'); const target = treeTarget ?? treeSelectTarget ?? cascaderTarget; target?.click(); await window.__d4NextTick?.(); await new Promise(resolve => requestAnimationFrame(() => requestAnimationFrame(resolve))); return { html: app?.innerHTML ?? '', before, ids, interacted: Boolean(target), changed: before !== (app?.innerHTML ?? '') || beforeBody !== document.body.innerHTML || target?.getAttribute('aria-expanded') === 'true', businessEvents: window.__d4EventLog?.length ?? 0 } })
