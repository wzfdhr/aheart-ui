...
状态：用户已明确要求“继续执行d5”，已启动基线核对和方案准备。以下新增公共API与行为迁移方案尚待专项批准；未修改组件实现，不把启动阶段视为方案已全部获批。
...
推荐先执行D5-A：接受上述可选dataMode与旧行为兼容路径、受控空排序null、行选择扩展、分页透传和统一归一化。B/C继续保留待评审，不自动扩大本批范围。
